...
class Transfer_cr extends CI_Controller {
    ...
    public function index(){
        ...
        $data['page']='transfercr/index.php';
    ...
    public function create(){
		...
		if($this->form_validation->run()==FALSE){
			$data['page']='transfercr/add.php';
            $this->load->view('master',$data);
		}else{
            $data['amount_cr']=$this->input->post('transfer_amount',true);
            $data['account_no']=$this->input->post('from_account',true);
            $data['type']=4;
            $data['trans_at']=date('Y-m-d H:i:s');
			$data['note']="Bank credited customer account.";
